...
- **Exclude an account from metrics** — `PATCH /admin/users/{id}/metrics-exclusion` body `{ "excluded": true|false }` → `AdminUserDetailResponse`. Hides a store-review / robo test account from ALL metrics without deleting it. `excludedFromMetrics` also appears on `AdminUserSummaryResponse` (the list) and `AdminUserDetailResponse`.
