...
- **Market intel by chain** — `/admin/market-intel` returns `topMarketsByChain` (grouped by `marketName` — unifies multi-unit chains) alongside `topMarkets` (one row per CNPJ). Same `{ cnpj, name, scans, spend }` shape; on the by-chain list `cnpj` is one representative unit.
