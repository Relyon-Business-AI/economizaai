 ...
 * ({@code economizai.categorizer.maintenance.*}) so it can be disabled or retimed.
 */
@Slf4j
@Component
...
public class CategorizerMaintenanceJob {
    ...
    @Value("${economizai.categorizer.maintenance.enabled:true}")
    private boolean enabled;

    @Scheduled(cron = "${economizai.categorizer.maintenance.cron:0 40 4 * * *}",
            zone = "${economizai.categorizer.maintenance.zone:America/Sao_Paulo}")
